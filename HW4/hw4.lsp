...
;The SAT encoding of the At Most One constraint is: AMO(X) = {!xi V !xj | xi,xj in X, i<j}

;create helper function to generate clauses of negated variables  (!xi V !xj)
(defun amoc-helper (n c k i)
    (cond ((> i k) NIL)
    (t (append (list (list (* -1 (node2var n c k)) (* -1 (node2var n i k)))) (amoc-helper n c k (+ i 1))))))
;combine clauses generated by helper function (helper clause 1 OR helper clause 2 OR ...)
(defun at-most-one-color (n c k)
    (cond ((= c k) NIL)
    (t (append (amoc-helper n c k (+ c 1)) (at-most-one-color n (+ c 1) k)))))
...
;
;For x and y in edge,  get their at-least-one-color lists, and then combine the negated clauses 
(defun edge-helper (a b)
    (cond ((or (NOT a) (NOT b)) '())
    (t (cons (list (* -1 (car a)) (* -1 (car b))) (edge-helper (cdr a) (cdr b))))))
(defun generate-edge-clauses (e k)
    (edge-helper (at-least-one-color (car e) 1 k) (at-least-one-color (cadr e) 1 k)))
